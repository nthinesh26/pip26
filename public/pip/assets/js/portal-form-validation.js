(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function validateControl(el) {
      ...
      // Date (Laravel-safe): validate ISO + real calendar date.
      // IMPORTANT: do NOT rely on el.checkValidity() alone because a previous customValidity
      // (e.g., required error) can make checkValidity() fail even after the user fixes the value.
      if (type === "date") {
        const raw = norm(el.value);

        // If optional and empty -> neutral/valid
        if (!el.required && raw === "") {
          clearNeutral(el);
          return true;
        }

        // Required is already enforced above; but ensure we don't keep stale customValidity
        el.setCustomValidity("");
        ...
        // Must be ISO after normalization
        if (!isISODate(raw)) {
          paintInvalid(el, t("invalid_date", { label }) || `${label}: invalid date`);
          return false;
        }

        // Must be a real date (no NaN)
        ...
        // Cross-check parts to avoid JS auto-rollover (e.g., 2026-02-31 becomes March)
      ...
      // ✅ FIX: maxlength is NOT treated as a "format constraint" for custom format_check
      // (maxlength is already enforced by the browser and should not trigger false failures)
